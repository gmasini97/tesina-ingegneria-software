...
La gerarchia di dispositivi è gestita sottoforma di albero, ovvero ogni dispositivo può essere aggregato in un gruppi che può a sua volta appartenere ad un'altro gruppo. Non sono ammesse relazioni ricorsive tra gruppi. Tutti i gruppi/dispositivi sono figli (diretti o indiretti) di un gruppo radice. In \hyperref[fig:gerarchia]{figura \ref{fig:gerarchia}} si può osservare un esempio di organizzazione gerarchica di dispositivi e gruppi.
...
\begin{itemize}
    ...
    \item Le funzioni e i metodi che \textit{ffmpeg} mette a disposizione.
...
Il progetto dipende dai progetti \textit{MariaDB} e \textit{ffmpeg}, utilizza il protocollo ip e i protocolli che si appoggiano ad esso (http, udp, rtsp etc) e supporta solo macchine \textit{x86} e \textit{amd64} con sistema operativo Microsoft Windows, Linux, maxOS. I browser supportati dall'interfaccia web sono Microsoft Edge, Apple Safari, Mozilla Firefox, Google Chrome (e suoi derivati, es: Chromium, Opera etc).
...
\begin{center}
    \begin{tabularx}{\linewidth}{|l|l|X|}
        ...
        \hline
            Descrizione & \multicolumn{2}{X|}{L'applicazione riceve in formato JSON la lista di dispositivi, gruppi e la loro organizzazione gerarchica.} \\
        \hline
            Formato dati & \multicolumn{2}{X|}{
                \begin{lstlisting}{language=JSON}^^J
                    \{^^J
                        \{^^J
                            "nome": "dispositivo 1",^^J
                            "configurazione": \{^^J
                                ...^^J
                            \}^^J
                        \},^^J
...
\begin{center}
    \begin{tabularx}{\linewidth}{|l|l|X|}
        \hline
            RF01 & Subsys & Func \\
        \hline
            Input & \multicolumn{2}{l|}{input descr} \\
        \hline
            Output & \multicolumn{2}{l|}{input descr} \\
        \hline
    \end{tabularx}
\end{center}
