...
\newcommand{\srsnomeprogetto}{Progetto Senza Titolo}
...
Il software utilizza un database \textit{nosql} per la memorizzazione dei dati degli utenti e delle risorse. Per quanto concerne l'elaborazione degli \textit{stream} audio e video, l'applicazione si avvale di una libreria esterna. L'applicazione necessita di almeno una interfaccia di rete installata e configurata correttamente sulla macchina \textit{Server} per la comunicazione coi dispositivi da monitorare, l'accesso da parte dei \textit{Client} e la connessione al servizio di database (nel caso questo sia installato su una macchina remota).
...
Di seguito viene mostrato un possibile sequence diagram per la risoluzione di una espressione da parte dell'interpreter; inoltre viene proposto in pseudo codice Java una possibile implementazione di alcune classi dello stesso.

\begin{figure}[H]
    \caption{Object diagram di una espressione d'esempio e sequence diagram della risoluzione della espressione con valore di $x=100$}
    \centering
    \includegraphics[width=\textwidth]{4-interpreterexample}
    \label{fig:interpreterexample}
\end{figure}
%}
